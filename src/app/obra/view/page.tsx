...
function ObraViewContent() {
  ...
  // Validación de autorización estricta v3.3.4
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // El login automático manejará la redirección o actualización de estado
    ...
    if (!success) {
      toast({
        ...
        description: "Acceso denegado para este proyecto técnico.",
      });
    } else {
      toast({
        title: "Identidad Validada",
        description: "Sincronizando con Tamer Cloud v3.3.4...",
      });
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#f1f5f9]">
        <Card className="max-w-md w-full p-10 text-center rounded-[2.5rem] shadow-2xl">
          ...
          <p className="text-muted-foreground mb-6">Por favor, utilice un código QR autorizado de Tamer Industrial.</p>
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center font-black">VALIDANDO SEGURIDAD v3.3.4...</div>;
  ...
  if (!obra || error) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#f1f5f9]">
        <Card className="max-w-md w-full p-10 text-center rounded-[2.5rem] shadow-2xl">
          <AlertCircle className="w-16 h-16 text-destructive mx-auto mb-6" />
          <h1 className="text-2xl font-black text-[#0a3d62]">Obra Desactivada</h1>
          <p className="text-muted-foreground mb-6">Este registro técnico no está disponible actualmente.</p>
      ...
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-4">
        <Card className="max-w-md w-full border-none shadow-2xl rounded-[3rem] overflow-hidden bg-white">
          <div className="bg-[#0a3d62] p-10 text-center text-white relative">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter leading-none">Acceso Documental</h2>
            <p className="text-white/60 text-[10px] font-black uppercase tracking-[0.3em] mt-3">Tamer Industrial S.A. v3.3.4</p>
          </div>
          <CardContent className="p-10">
            <div className="mb-8 p-6 bg-primary/5 rounded-[2rem] border border-primary/10">
              <p className="text-[10px] font-black text-primary uppercase tracking-widest mb-1">PROYECTO TÉCNICO</p>
            ...
            {isUser && (
              <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-3">
                ...
                <p className="text-[10px] text-amber-800 font-bold uppercase">Su sesión actual no está autorizada para esta obra específica.</p>
                <Button variant="ghost" size="icon" onClick={logout} className="rounded-xl ml-auto"><LogOut className="w-4 h-4" /></Button>
              </div>
            )}

            <form onSubmit={handleLogin} className="space-y-5">
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-2 tracking-widest">Email Autorizado</Label>
                <Input 
                  value={identifier}
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-muted-foreground ml-2 tracking-widest">Clave de Seguridad</Label>
              ...
              >
                {isLoggingIn ? <Loader2 className="animate-spin w-6 h-6" /> : <><ShieldCheck className="w-6 h-6" /> VALIDAR ACCESO TÉCNICO</>}
              </Button>
            </form>
          </CardContent>
          <div className="px-10 pb-10">
            <p className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] text-center leading-relaxed">
              CONFIDENCIAL: TODO ACCESO ES REGISTRADO v3.3.4
            </p>
          </div>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-[#f1f5f9] pb-20 font-body">
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8 relative z-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[10px] font-black opacity-60 tracking-[0.5em] uppercase mt-2">VISOR TÉCNICO AUTORIZADO v3.3.4</p>
            </div>
          </div>
          <div className="flex flex-col items-end gap-3">
            ...
            <Button variant="ghost" onClick={logout} className="text-white/60 hover:text-white hover:bg-white/10 font-black text-[10px] tracking-widest">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-8 -mt-12 space-y-10 relative z-20">
          ...
          <CardHeader className="p-10 border-b bg-gray-50/50">
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="text-center md:text-left">
                ...
                <CardDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground mt-1">Ingeniería Tamer Industrial</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent className="p-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Ubicación</p>
                    <p className="font-bold text-lg">{obra.direccion || 'Instalación Técnica'}</p>
                  </div>
                </div>
              </div>
              <div className="bg-[#f8fafc] p-6 rounded-[2rem] border border-secondary flex flex-col justify-center">
                <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-2 flex items-center gap-2">
                  <Info className="w-3 h-3" /> Estado de Documentación
                </p>
                <p className="text-xs font-medium text-gray-700 leading-relaxed italic">
                  {obra.descripcion || 'Toda la documentación visualizada en este portal es de carácter oficial y está vigente para la ejecución de tareas en campo.'}
                </p>
              </div>
            </div>

            {/* Acceso a Carpeta de Drive v3.3.4 */}
            {obra.driveFolderUrl && (
              <div className="mt-10 pt-10 border-t">
                ...
                >
                  <a href={obra.driveFolderUrl} target="_blank" rel="noopener noreferrer">
                    <FolderOpen className="w-8 h-8" /> ACCEDER A CARPETA DE PLANOS (DRIVE)
                  </a>
                </Button>
                <p className="text-center text-[10px] font-black text-emerald-600/60 uppercase tracking-widest mt-4">
                  ACCESO EXTERNO AUTORIZADO A CARPETA DE INGENIERÍA
                </p>
              </div>
            )}
          </CardContent>
        ...
        <div className="space-y-6">
          <div className="px-6 flex items-center justify-between">
            <h3 className="text-xs font-black text-[#0a3d62] uppercase tracking-[0.4em] flex items-center gap-3">
              <FileText className="w-5 h-5 text-primary" /> Archivos Individuales
            </h3>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {obra.files && obra.files.length > 0 ? (
              ...
              <div className="p-20 text-center border-4 border-dashed rounded-[3rem] border-secondary bg-white/50">
                <p className="text-muted-foreground font-black uppercase tracking-widest text-xs">Sin archivos individuales cargados.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <footer className="pt-10 text-center text-[10px] font-black text-muted-foreground/40 uppercase tracking-widest">
          © {new Date().getFullYear()} {empresa?.nombre || 'TAMER INDUSTRIAL S.A.'} | v3.3.4
        </footer>
      </div>

      <Dialog open={!!selectedFile} onOpenChange={(open) => !open && setSelectedFile(null)}>
        <DialogContent className="max-w-[90vw] w-full h-[85vh] p-0 rounded-[2.5rem] overflow-hidden">
          <div className="bg-[#0a3d62] text-white p-6 font-black uppercase tracking-tighter truncate">
            {selectedFile}
          </div>
          <div className="flex-1 bg-gray-100 flex flex-col items-center justify-center space-y-6">
            <Loader2 className="w-12 h-12 animate-spin text-primary" />
            <p className="text-sm font-black uppercase tracking-widest text-[#0a3d62]">Conectando con Servidor de Archivos...</p>
            <Button className="rounded-2xl h-14 px-10 font-black">DESCARGAR PARA VISUALIZAR</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
...
export default function ObraViewPage() {
  return (
    <Suspense fallback={<div className="p-20 text-center font-black">Sincronizando v3.3.4...</div>}>
      <ObraViewContent />
    </Suspense>
  );
}
