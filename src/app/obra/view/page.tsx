...
function ObraViewContent() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#0a3d62]">Cargando Documentación...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-slate-100">
        <Card className="max-w-md w-full p-8 text-center rounded-[2rem] shadow-2xl border-none">
          ...
          <p className="text-xs text-muted-foreground mt-2 mb-8 italic">El enlace QR no es válido o la obra fue removida.</p>
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <Card className="max-w-md w-full border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-white">
          <div className="bg-[#0a3d62] p-8 text-center text-white">
            ...
            <h2 className="text-lg font-black uppercase tracking-tight">Acceso Restringido</h2>
            <p className="text-[9px] font-black opacity-40 uppercase tracking-[0.3em] mt-2">Validación de Seguridad</p>
          </div>
          <CardContent className="p-6">
            <div className="mb-6 p-4 bg-blue-50 rounded-2xl border border-blue-100 text-center">
              ...
              <h3 className="font-black text-lg text-[#0a3d62] uppercase truncate mt-1">{obra.nombreObra}</h3>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              >
                {isLoggingIn ? <Loader2 className="animate-spin" /> : <><ShieldCheck className="w-5 h-5" /> DESBLOQUEAR OBRA</>}
              </Button>
            </form>
          </CardContent>
        </Card>
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-20 overflow-x-hidden">
      <div className="bg-[#0a3d62] text-white pt-12 pb-20 px-4 relative">
        <div className="max-w-2xl mx-auto flex flex-col items-center text-center gap-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[8px] font-black opacity-40 tracking-[0.3em] uppercase">Documentación Técnica</p>
          </div>
          <Button variant="outline" size="sm" onClick={logout} className="bg-white/10 border-white/20 text-white hover:bg-white/20 rounded-full h-9 px-6 font-black text-[9px] uppercase tracking-widest mt-2">
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-12 space-y-6">
          ...
          <CardHeader className="p-6 border-b bg-gray-50/50">
            <div className="flex items-center gap-4">
              ...
              <div className="min-w-0">
                <CardTitle className="text-base sm:text-lg font-black text-[#0a3d62] uppercase truncate leading-none mb-1">
                  {obra.nombreObra}
                </CardTitle>
                <div className="flex gap-2">
                  <Badge className="bg-primary/10 text-primary border-none font-black text-[8px] px-2 py-0.5">OF: {obra.numeroOF}</Badge>
                  <Badge className="bg-primary/10 text-primary border-none font-black text-[8px] px-2 py-0.5">OT: {obra.numeroOT}</Badge>
                </div>
              </div>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-6">
              <div className="space-y-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[9px] font-black uppercase text-muted-foreground mb-0.5 tracking-widest">Ubicación</p>
                    <p className="font-bold text-[#0a3d62] text-xs">{obra.direccion || 'No especificada'}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  <Info className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                  <div>
                    <p className="text-[9px] font-black uppercase text-muted-foreground mb-0.5 tracking-widest">Descripción</p>
                    <p className="text-[10px] text-gray-600 leading-relaxed font-medium">{obra.descripcion || 'Sin descripción adicional.'}</p>
                  </div>
                </div>
              </div>

              {obra.driveFolderUrl && (
                <div className="bg-secondary/20 p-4 rounded-2xl border border-secondary flex flex-col items-center gap-3">
                  <FolderOpen className="w-8 h-8 text-emerald-600" />
                  <p className="text-[10px] font-black text-[#0a3d62] uppercase text-center">Carpeta Completa de Planos</p>
                  <Button asChild className="w-full h-12 bg-emerald-600 hover:bg-emerald-700 rounded-xl text-sm font-black gap-2 shadow-md">
        ...
        <div className="space-y-3 px-1">
          <h3 className="text-[10px] font-black text-[#0a3d62] uppercase tracking-[0.2em] flex items-center gap-2">
            <FileText className="w-4 h-4 text-primary" /> Lista de Archivos
          </h3>
          
          <div className="grid grid-cols-1 gap-2">
            {obra.files && obra.files.length > 0 ? (
              obra.files.map((fileName, idx) => (
                <div key={idx} className="bg-white p-4 rounded-xl shadow-sm border border-secondary flex items-center justify-between gap-3 group">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-8 h-8 bg-slate-50 rounded-lg flex items-center justify-center text-primary shrink-0">
                      <FileText className="w-4 h-4" />
                    </div>
                    <p className="font-bold text-[#0a3d62] text-xs truncate">{fileName}</p>
                  </div>
                  {obra.driveFolderUrl && (
              ...
              <div className="p-8 text-center border-2 border-dashed rounded-2xl border-slate-200 bg-white/50">
                <p className="text-muted-foreground font-black uppercase tracking-widest text-[8px]">No hay archivos cargados.</p>
              </div>
            )}
          </div>
        </div>
